...
public class RegisterFace extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        kGradientPanel3.add(txt_student_id, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 0, 160, 20));
        ...
        kGradientPanel4.add(txt_class_section, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 0, 160, 20));
    ...
    private int getIdUser() {
        int id = 0;
        conecta.connect();
        conecta.executaSQL("SELECT max(id) + 1 id FROM person ORDER BY id DESC LIMIT 1");
        try {
            if (conecta.rs.next()) {
                id = Integer.parseInt(conecta.rs.getString("id"));
            }           
                            ...
                            for (int i = 0; i < detectedFaces.size(); i++) { //repetição pra encontrar as faces
                                ...
                                if (saveButton.getModel().isPressed()) { //quando apertar o botão saveButton
                                    if (txt_first_name.getText().equals("") || txt_first_name.getText().equals(" ")) {
                                        ...
                                        JOptionPane.showMessageDialog(null, "Empty field");
                                    } else if (txt_student_id.getText().equals("") || txt_student_id.getText().equals(" ")) {
                                        JOptionPane.showMessageDialog(null, "Empty field");
                                    } else if (txt_class_section.getText().equals("") || txt_class_section.getText().equals(" ")) {
                                        JOptionPane.showMessageDialog(null, "Empty field");
                                    } else {
...
//                                        nome do arquivo: idpessoa + a contagem de fotos. ex: person.10(id).6(sexta foto).jpg
                            ...
                            try {
                                if (g.drawImage(buff, 0, 0, 360, 390, 0, 0, buff.getWidth(), buff.getHeight(), null)) {
                                    if (runnable == false) {
                                        System.out.println("Hello to Photos");
                                        this.wait();
                                    }
                                }
                            } catch (Exception e) {
